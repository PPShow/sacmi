...
public class ExcelReaderTools {
	...
	private void validator(String filePath, Character inputMode) throws Exception {
		if(filePath == null || "".equals(filePath)) {
			throw new ExcelToolsException("Caminho para arquivo inválido");
		}
		...
		if( filePath.contains(".xlsx") ) {
			...
			throw new ExcelToolsException("Tipo de arquivo não é do tipo excel");
		}
		
		if(!INPUT_MODE_FILE.equals(inputMode) && !INPUT_MODE_INPUT_STREAM.equals(inputMode) ) {
			throw new ExcelToolsException("Formato de input não suportado");
		}
	}
